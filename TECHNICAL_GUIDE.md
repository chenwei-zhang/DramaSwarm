...
> 多智能体群体仿真引擎：从知识图谱到危机推演

---

## 一、系统概述

DramaSwarm 是一个面向中国娱乐圈的多智能体仿真系统。它用 **知识图谱** 存储明星关系、八卦事件、新闻报道等结构化数据，在此基础上运行两种仿真引擎：

1. **综艺修罗场引擎** — 多个 Agent 在虚拟综艺节目中自由互动，触发五层链式反应（舆论→媒体→热搜→监管→品牌）
2. **危机仿真引擎** — 从历史八卦事件中加载场景，多个明星 Agent 顺序决策、观众 Agent 生成评论、关系类型影响行为选择

两个引擎共享底层 Agent 架构和 LLM 集成层，但拥有独立的环境模型和状态管理。
...
```
Agent (基类)
├── SimpleAgent — 纯规则，从角色模板池中随机选取回复，零延迟
└── LLMAgent — 调用 LLM 生成回复，带 conversation history，失败时 fallback 到规则模式
```
...
六个预设角色，每个角色有独立的中文回复模板池：

| 角色 | 标签 | 决策特征 |
|------|------|----------|
| LEADER | 组长 | 发号施令，组织协调 |
| PEACEMAKER | 和事佬 | 调解冲突，倾向妥协 |
| DRAMA_QUEEN | 戏精 | 情绪夸张，制造话题 |
| SLACKER | 摸鱼王 | 消极被动，少参与 |
| PERFECTIONIST | 完美主义 | 挑剔细节，高标准 |
| WILDCARD | 变数 | 不可预测，打破常规 |

### 3.4 Big Five 性格模型

LLMAgent 的 `PersonalityProfile` 使用大五人格模型：
...
事件循环是仿真的"心跳"，每回合执行六个阶段：

```
┌──────────────────────────────────────────────────────────┐
│  Turn N                                                   │
│                                                           │
│  1. PREPARATION   → 从 Agent 池随机选 3 个激活             │
│  2. PERCEPTION    → 注入环境描述 + 个性化反应上下文         │
│  3. DECISION      → 每个 Agent 调用 think()               │
│  4. ACTION        → 随机决策: 60%发言 / 20%行动 / 20%等待  │
│                     ↓ speak/act 结果通过 ↓                 │
│                     environment.add_event() 触发五层反应   │
│  5. REFLECTION    → 全体 Agent memory.advance_turn()       │
│  6. CLEANUP       → 截断历史 + reaction_bus.decay()        │
│                                                           │
│  → environment.advance_turn() → 时间+10min + 快照          │
└──────────────────────────────────────────────────────────┘
```

### 4.2 严重度估算

`_estimate_severity()` 使用关键词匹配计算事件严重度：

```python
base = 0.4
for keyword in NEGATIVE_KEYWORDS:  # "出轨", "封杀", "吸毒" 等 12 个
    if keyword in content: base += 0.15
for keyword in POSITIVE_KEYWORDS:  # "感谢", "支持", "合作" 等 6 个
    if keyword in content: base -= 0.15
if knowledge_graph:
    impact = kg.get_event_impact(content, agent_name)
    base += impact.severity_delta
return clamp(base, 0.1, 1.0)
```

### 4.3 反重复机制

`_is_repetitive()` 维护最近 10 条发言的滑动窗口：

1. **精确匹配**：忽略大小写的完整匹配
2. **词重叠检测**：Jaccard 相似度 `|A∩B| / max(|A|,|B|)`，阈值 0.7

重复发言被替换为 `[{agent.name} 似乎想说什么但没说出口]`。

### 4.4 事件循环变体

| 变体 | 特点 |
|------|------|
| `EventLoop` | 标准循环，随机顺序，60/20/20 行动分配 |
| `SequentialEventLoop` | Agent 按序发言，后发言者能听到前面人的话 |
| `InteractiveEventLoop` | 支持外部事件注入（`inject_event()` 排队，在 ACTION 阶段前执行） |

---

## 五、五层链式反应系统

这是综艺引擎的核心创新——事件经过五个按优先级排列的反应层，每层读取上游层输出的上下文：

```
事件描述 + severity + source
        │
        ▼
┌─ PublicOpinionLayer ─────────────────────────────┐
│  关键词匹配 → sentiment shift → approval 更新     │
│  粉丝反应生成（DISAPPOINTED/DEFENSIVE/ANGRY/...）  │
│  输出: approval_scores → 注入到下游上下文          │
└───────────────────────────────────────────────────┘
        │ approval_scores
        ▼
┌─ MediaLayer ──────────────────────────────────────┐
│  随机选 1-3 家媒体 → 模板生成标题                    │
│  小报放大系数 1.8x / 主流媒体 1.0x / 财经 0.7x     │
│  风格修饰: "夸张"→ 加!!!  "煽情"→ 加"独家："        │
└───────────────────────────────────────────────────┘
        │
        ▼
┌─ SocialPlatformLayer ────────────────────────────┐
│  severity >= 0.3 才生成热搜                        │
│  热度 = severity × random(50000, 200000)           │
│  相似话题合并（前4字符匹配）→ boost 热度             │
│  排行榜最多 50 条，每日衰减 20%                     │
└───────────────────────────────────────────────────┘
        │
        ▼
┌─ GovernmentLayer ─────────────────────────────────┐
│  5 级敏感关键词扫描                                  │
│  Lv1 "不专业" → KEYWORD_WARNING                     │
│  Lv2 "出轨/劈腿" → CONTENT_REMOVAL                 │
│  Lv3 "吸毒/偷税" → ACCOUNT_RESTRICTION             │
│  Lv4 "性侵" → OFFICIAL_WARNING                     │
│  Lv5 "分裂国家" → PERMANENT_BAN                    │
│  ★ 触发关键词 → social_platform.censor_topic() ★   │
└───────────────────────────────────────────────────┘
        │
        ▼
┌─ CommercialLayer ─────────────────────────────────┐
│  读取 PublicOpinion 的 approval_scores             │
│  商业价值 = approval × 0.6 + 20                    │
│  品牌决策: CONTINUE → MONITORING → NEGOTIATING     │
│           → SUSPENDED → TERMINATED                 │
│  解约触发: value_drop > 30 或 severity > 0.8       │
└───────────────────────────────────────────────────┘
```

### 跨层效应

1. **PublicOpinion → Commercial**：品牌层直接读取公众舆论的口碑分来决策
2. **Government → SocialPlatform**：监管层扫描到敏感关键词后，调用 `censor_topic()` 将相关热搜热度降低 70%

### 快照系统

每回合结束时，`ReactionBus.take_snapshot()` 深拷贝所有五层状态，形成时间序列。快照上限 200 条（滚动窗口），支持历史回溯分析。
...
CrisisScenarioEngine          CrisisSimulation
  └─ 从图谱加载场景             └─ 异步运行仿真循环
       │                              │
  TemporalKnowledgeGraph        CrisisTimeline (1天=1回合)
  list_crisis_scenarios()       6阶段生命周期
  ...
  Big Five 性格               粉丝/路人/理中客/黑粉        强制动作/外部事件
  规则/LLM 双模式             模板评论生成                    │
       │                            │                             │
       └──────────── MessageBus ──────────────────────────────────┘
                           (Agent 间消息总线)
...
`CrisisSimulation.step()` 的核心改动是 Agent 的执行方式从并行改为顺序：
...
```
条件: 关系 ∈ (配偶, 前配偶, 家人) AND 动作 ∈ (道歉, 反击, 卖惨, 声明)
效果: 在目标 Agent 的 memory 中追加 PROPAGATION 标记

条件: 关系 ∈ (对手, 竞争对手) AND 动作 ∈ (反击, 卖惨)
效果: 同上
```
...
仿真中的自然衰减模拟了"时间是最好的公关"这一现实规律：

```
热度: heat *= 0.9              # 每天自然降 10%
口碑: score → 50 方向回归       # 低于 50 每天回升 0.5，高于 50 每天下降 0.3
...
- **后端**: FastAPI + uvicorn，三个 API 路由（graph/sim/crisis）
...
| 模式 | 应用位置 | 说明 |
|------|---------|------|
| 管道/责任链 | ReactionBus | 五层顺序处理，上下文逐层传递 |
| 策略 | SimpleAgent vs LLMAgent | 统一接口，不同决策策略 |
| 观察者 | Observer + EventLoop hooks | 监控 Agent/环境状态变化 |
| 模板方法 | `_run_turn()` 骨架 | 子类覆盖单个阶段 |
| 工厂 | AgentFactory + VarietyShowFactory | 模板化创建 Agent |
| 备忘录 | ReactionBus.take_snapshot() | 五层状态完整快照 |
...
```
tests/
├── test_agent.py             # 26 个测试 — Agent 生命周期、记忆系统、LLM fallback
